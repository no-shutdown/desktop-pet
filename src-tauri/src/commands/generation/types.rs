...
const STATE_DEFINITIONS: [StateDefinition; 4] = [
    ...
    StateDefinition {
        ...
        requirements: "The character MUST be SEATED behind a desk with one open laptop computer in every single frame, awake with eyes OPEN and head UP. The laptop has a clearly visible hinged screen panel and keyboard deck, rests flat on the tabletop, and both hands type on the laptop keyboard. The desk/table is fixed lower-third at elbow height with the same desk geometry in all frames. Explicitly forbid a standalone keyboard, keyboard-only output, tablet, closed laptop, laptop on the character's lap (lap-held), lap-held laptop, held device, or independent monitor substitution. No desk/chair/laptop drift or replacement, no UI, screen content, code, papers, symbols, floating props, or detached effects. Camera is fixed; the character and the furniture stay absolutely centered and do not translate horizontally or vertically between frames.",
    },
];
